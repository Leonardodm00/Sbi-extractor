...
| 2026-09-21 | v8.2b. The array-dependency probe is rewritten after its first run produced unreadable evidence. Two faults, both of its own making: all three subjobs shared one `#PBS -o` path and overwrote each other from byte zero, and state was cleared at SUBMIT time so a still-running previous array wrote its `.ok` files into the new run's evidence. Each run now lives in `out/probe/<RUN>/`, each subjob writes its own log, a new run refuses to start while any probe job is queued, and `check` prints a computed VERDICT (`GATED` / `NOT GATED` / `INCONCLUSIVE`) per run instead of files to eyeball. A third bug was found by simulating the scheduler both ways before shipping: `ls <glob> | wc -l` under `set -eo pipefail` kills the aggregation job when the glob matches nothing, which is the NORMAL case, and would have shown up as gating that never happened. **[OPEN]** whether `afterok:<array id>` withholds the dependent after a subjob exits 1 on davinci is still unanswered; note that `cohort_manifest.py` refuses a partial cohort regardless (M9, M14), so the dependency is ordering and the manifest builder is the safety net. |
| 2026-09-21 | v8.2a, after the first Stage D run on davinci. **Correction:** `afterokarray` is a TORQUE dependency type; PBS Pro has no `*array` variants and rejects the whole `-W` value (`qsub: illegal -W value`), so the aggregation job was never submittable as written. An array dependency in PBS Pro uses the ORDINARY keyword with an array job id carrying the brackets. Swept on davinci against a held array: `afterok:<id>[].pbsserver01` and `afterany:<id>[].pbsserver01` ACCEPTED, `afterokarray:<id>[].pbsserver01` REJECTED, `afterok:<id>[]` ACCEPTED (server suffix optional). `launch_stage_d.sh` and the probe now use `afterok:<array id>`; `probe_afterokarray*` renamed `probe_array_depend*` because the old name states a keyword this scheduler does not have. Login-node gates all PASS on davinci [CLUSTER 2026-09-21]: `cohort_config` loads with `torch imported: False`, `extraction_flags.sh` regenerates byte-identical at 35 wells, `smoke_test_cohort_manifest.py` 14/14, export suite `PASSED 17` with T9's census `n=36 |L|=26 active=23 p=27` IDENTICAL to the 09-20 record. |
| 2026-09-21 | v8.2, Stage D built (not yet run on the cluster). The cohort block is now ONE definition, `hpc/dsn/cohort.py` in the SBI repo (torch-free; `config.CohortConfig` re-exports it), read here through `cohort_config.py`, so `extractor/list_extraction_jobs.py` no longer imports torch. Extractor version 3 writes the eight preprocessing fields, the extractor commit and `manifest_version` into every archive and `traces_meta.json`; `cohort_manifest.py` builds ONE `cohort_manifest.json` from the fragments or refuses (constancy across wells, measured == configured, `n_units == n_wells x n_subsets`, tracked flags); `run_cohort_manifest.pbs` runs it under `depend=afterok:<array id>`, `extractor/launch_stage_d.sh` submits both and refuses the config's own `extract_root`. `dsn_frozen.py` no longer falls back to `w_size=0.02 / gaussian_window=0.04` when a checkpoint lacks `cohort.*`: it raises (export suite T8b). `mfr_threshold` and `n_subsets` are RECORDED on the sim arm and never applied to it (decision 2026-09-21). New suite `smoke_test_cohort_manifest.py` (14). Sec. 2 rows added; the Stage D chain and its cluster commands are in `extractor/README.md`, sec. "Stage D". |
...
| `cohort_manifest.py` | **v8.2.** Stage D aggregation: `build_manifest()` over the per-well `traces_meta.json` fragments -> `cohort_manifest.json` (+ `.sha256`), or `ManifestError`; `assert_archive_matches_manifest()` (real arm), `sim_preprocessing_from_manifest()` / `assert_sim_geometry()` (sim arm); a version-1 archive is `LegacyArchive`. CLI: `--config --manifest --flags --extract-root --out --check-only` |
| `smoke_test_cohort_manifest.py` | **v8.2.** 14 checks on a synthetic 3-well cohort pushed through the REAL extractor chain by subprocess, 8 of them refusals (M7-M14: sim geometry, a well drifting from the others, a missing fragment, measured != configured, an archive tampered against the manifest, a legacy version-1 archive, drifted flags, a well with the wrong archive count) |
